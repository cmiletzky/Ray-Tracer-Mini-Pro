	...
	public void testGetNormal() {
	 ...
	 @Test
	    public void testfindIntersections() {
	        ...
	        List<Point3D> result = plane.findIntersectionsPoints(new Ray(new Point3D(0,4,0), new Vector(-4,-3,0)));
	        assertEquals("Wrong number of points", 1, result.size());
	        assertEquals("Ray intersects the plane", List.of(p), result);

	        // TC02: Ray does not intersect the plane (0 points)
	        assertNull("Ray starts after plane",
	                plane.findIntersectionsPoints(new Ray(new Point3D(-4,1,0),new Vector(-4,-3,0))));
	        ...
	        assertNull("Ray included in plane",
	                plane.findIntersectionsPoints(new Ray(new Point3D(0.657087677596453,-1.051994262427145,2.394906584830692),
	        ...
	        assertNull("Ray parallel with plane",
	                plane.findIntersectionsPoints(new Ray(new Point3D(0,4,0), new Vector(-2,0,2))));
	        // **** Group: Ray is orthogonal to the plane
	        // TC13: – according to p0 (before) (1 point)
	        p = new Point3D(0.33333333333333333,1.33333333333333333,0.33333333333333333);
	        result = plane.findIntersectionsPoints(new Ray(new Point3D(0,1,0), new Vector(1,1,1)));
	        assertEquals("Wrong number of points", 1, result.size());
	        assertEquals("Ray intersects the plane", List.of(p), result);
	        // TC14: – according to p0 (in) (0 points)
	        assertNull("Ray starts at plane",
	                plane.findIntersectionsPoints(new Ray(new Point3D(0,2,0), new Vector(1,1,1))));
	        // TC15: – according to p0 (after the plane) (0 points)
	        assertNull("Ray starts after plane",
	                plane.findIntersectionsPoints(new Ray(new Point3D(0.327523993905339,-0.403942564427651,4.408486769939391),
	                        new Vector(1,1,1))));
	        // TC16: Ray is neither orthogonal nor parallel to and begins at the plane
	        //(p0 is in the plane, but not the ray) (0 points)
	        assertNull("Ray starts at plane",
	                plane.findIntersectionsPoints(new Ray(new Point3D(-2.227529894658485,-0.144919886756481,4.372449781414965),
	                        new Vector(0,1,1))));
	        ...
	        assertNull("Ray starts at plane",
	                plane.findIntersectionsPoints(new Ray(new Point3D(2,0,0), new Vector(0,1,1))));
	    }
}
